...
export default function IncidentPhotoEditScreen() {
    ...
    const fetchPhoto = async () => {
      try {
        const { data, error } = await supabase
          ...
          .select(
            'title, description, link_url, category, image_urls, uploader_id',
          )
          .eq('id', photoId)
          .single();
        ...
        if (data.uploader_id !== user.id) {
          Alert.alert('권한 없음', '이 사진 자료를 수정할 권한이 없습니다.');
          navigation.goBack();
          return;
        }
        ...
        Alert.alert('오류', '사진 자료를 불러오는 데 실패했습니다.');
  ...
  const uploadToSupabase = async photo => {
    ...
    const storagePath = `incident-photos/${fileName}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('수정 완료', '사진 자료가 성공적으로 수정되었습니다.');
      navigation.goBack();
    } catch (err) {
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <Text style={styles.pageTitle}>사건 사진자료 수정</Text>
      <TextInput
        style={styles.input}
        placeholder="제목 *"
