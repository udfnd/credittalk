...
export default function ReviewEditScreen() {
    ...
    const fetchReview = async () => {
      try {
        const { data, error } = await supabase
          .from('reviews')
          .select('title, content, rating, image_urls, user_id')
          .eq('id', reviewId)
          .single();

        if (error) throw error;
        if (data.user_id !== user.id) {
          Alert.alert('권한 없음', '이 후기를 수정할 권한이 없습니다.');
          navigation.goBack();
          return;
        }
        ...
        Alert.alert('오류', '후기 정보를 불러오는 데 실패했습니다.');
  ...
  const uploadToSupabase = async photo => {
    ...
    const fileName = `reviews/${user.id}_${Date.now()}.${ext}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('수정 완료', '후기가 성공적으로 수정되었습니다.');
      navigation.goBack();
    } catch (err) {
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <Text style={styles.pageTitle}>후기 수정</Text>
      <TextInput
        style={styles.input}
        placeholder="제목 *"
      ...
      <StarEditor rating={rating} setRating={setRating} />
