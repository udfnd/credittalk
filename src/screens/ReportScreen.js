...
function ReportScreen({ navigation }) {
  ...
  const [showDamagedItemOtherInput, setShowDamagedItemOtherInput] =
    useState(false);
  const [cryptoTransferAmount, setCryptoTransferAmount] = useState('');
  const isNoShowCategory =
    category === '노쇼' || category === '노쇼 대리구매 사기';
  const faceToFaceLabel = isNoShowCategory ? '노쇼 피해' : '대면 피해';
  const faceToFaceCheckboxLabel = isNoShowCategory
    ? '대면 노쇼피해'
    : '기타로 피해 입음';
  ...
  const handleRemoveIllegalCollectionPhoto = uri => {
    setIllegalCollectionPhotos(prev => prev.filter(photo => photo.uri !== uri));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      if (
        category === '암호화폐' &&
        showDamagedItemOtherInput &&
        !damagedItem.trim()
      ) {
        Alert.alert(
          '입력 오류',
          '피해 물품(암호화폐 이름)을 직접 입력해주세요.',
        );
        return;
      }
    }
    ...
    try {
      ...
      const reportData = {
        ...
        crypto_transfer_amount: cryptoTransferAmount
          ? parseFloat(cryptoTransferAmount.replace(/,/g, ''))
          : null,
              ...
              <View style={styles.inputWithButtonContainer}>
                <TextInput
                  style={[
                    ...
                    showDamagedItemOtherInput && { backgroundColor: '#fff' },
                  ]}
                  value={damagedItem}
                  placeholder={
                    showDamagedItemOtherInput
  ...
  const damageAccountLabel =
    category === individualCategories[5] // "암호화폐" 카테고리
      ? '전자지갑주소 (선택)'
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.idInputContainer}>
          <TextInput
            style={[
              styles.input,
              { flex: 1 },
              isIdUnknown && styles.disabledInput,
            ]}
            value={perpetratorId}
          ...
          <TouchableOpacity
            ...
            <Icon
              name={isIdUnknown ? 'checkbox-marked' : 'checkbox-blank-outline'}
              size={24}
        ...
        <View style={styles.labelContainer}>
          <Text style={styles.label}>{damageAccountLabel}</Text>
        </View>
