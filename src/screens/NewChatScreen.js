...
function NewChatScreen() {
  ...
  const handleCreateOrGoToChat = async otherUser => {
    if (!user || !otherUser.auth_user_id) {
      Alert.alert(
        '오류',
        '사용자 정보가 올바르지 않아 채팅을 시작할 수 없습니다.',
      );
      return;
    }
    ...
    console.log(
      `[NewChatScreen] Attempting to create/go to chat with: ${otherUser.nickname} (ID: ${otherUser.auth_user_id})`,
    );
    ...
    try {
      ...
      if (functionData && functionData.roomId) {
        ...
        throw new Error(
          '채팅방 ID를 받아오지 못했습니다. 함수 응답을 확인해주세요.',
        );
      }
    } catch (error) {
