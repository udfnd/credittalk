...
function ChatListScreen() {
      ...
      const formattedRooms =
        data?.map(room => {
          ...
          if (!roomName && participants.length === 2) {
            ...
            participants[0].author_auth_id === user.id
          ) {
            roomName = '나와의 채팅';
          }
  ...
  useEffect(() => {
    ...
    const channels = supabase
      ...
      .on(
        ...
        payload => {
          console.log(
            'Chat room participants table change detected (chat_room_participants):',
            payload,
          );
          fetchChatRooms();
        },
  ...
  if (isLoading && chatRooms.length === 0 && !refreshing) {
    return (
      <ActivityIndicator style={styles.centered} size="large" color="#3d5afe" />
    );
  }
          ...
          <TouchableOpacity
            ...
            {item.latest_message_time && (
              <Text style={styles.lastMessageTime}>
                {new Date(item.latest_message_time).toLocaleTimeString([], {
                  hour: '2-digit',
                  minute: '2-digit',
                  hour12: false,
                })}
