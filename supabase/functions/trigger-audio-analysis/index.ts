...
async function getGoogleAuthToken(serviceAccountJson: string): Promise<string> {
  ...
  const signedJwt = `${signingInput}.${btoa(
...
serve(async req => {
  ...
  try {
    ...
    if (!gcpProjectId || !pubsubTopic || !googleCredentials) {
      throw new Error(
        'Missing required Google Cloud environment variables (GCP_PROJECT_ID, GCP_PUBSUB_TOPIC_ID, GOOGLE_CREDENTIALS).',
      );
    }
    ...
    console.log(
      `[TRIGGER] Publishing message for analysisId: ${analysisId} to ${pubsubUrl}`,
    );
    ...
    console.log(
      `[TRIGGER] Successfully triggered analysis for analysisId: ${analysisId}`,
    );
