...
async function verifyAdmin(supabaseAdminClient: SupabaseClient, authorizationHeader?: string | null): Promise<boolean> {
  // 방법 1: 요청 헤더의 JWT 토큰 검증 및 custom claims 확인
  // 방법 2: 별도의 관리자 API 키 확인
  // 방법 3: 관리자 테이블 조회 등
  console.warn("!!! Admin verification is NOT implemented. Access allowed for testing ONLY. !!!");
...
async function getDecryptedReport(supabaseAdminClient: SupabaseClient, reportId: number) {
  // 관리자용 DB 함수 호출
  const { data, error } = await supabaseAdminClient.rpc('get_decrypted_report_for_admin', {
    report_id_input: reportId
  });

  if (error) {
    console.error('Admin RPC error:', error);
    if (error.message.includes('Report not found')) throw new Error('Report not found.');
...
serve(async (req) => {
  ...
  try {
    ...
    // 관리자 권한 검증
    const isAdmin = await verifyAdmin(supabaseAdminClient, req.headers.get('Authorization')); // 실제 검증!
    if (!isAdmin) {
    ...
    const decryptedData = await getDecryptedReport(supabaseAdminClient, reportId);
