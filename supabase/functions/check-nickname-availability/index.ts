...
serve(async (req) => {
  ...
  try {
    ...
    if (!nickname || typeof nickname !== "string" || nickname.trim().length < 2) {
      ...
      return new Response(
        JSON.stringify({ error: "유효한 닉네임을 제공해야 합니다 (2자 이상)." }),
    ...
    // 1. 금지어 포함 여부 확인
    const hasForbiddenWord = forbiddenWords.some(word => lowercasedNickname.includes(word));
    if (hasForbiddenWord) {
