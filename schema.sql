...
CREATE OR REPLACE TRIGGER "on_new_comment_notification" AFTER INSERT ON "public"."comments" FOR EACH ROW EXECUTE FUNCTION "supabase_functions"."http_request"('https://<project_ref>.supabase.co/functions/v1/new-comment-notification', 'POST', '{"Content-Type": "application/json", "Authorization": "Bearer REMOVED_SUPABASE_SERVICE_ROLE_KEY"}', '{}', '1000');
